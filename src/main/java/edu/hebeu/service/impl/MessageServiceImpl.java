...
//        //2.云端私钥解密，拿到对称密钥
//
//        //3.使用对称密钥解密 加密信息，拿到明文消息、数字签名
//        ReturnValue rv = new ReturnValue(AESKey,secretMsg);
//        String s = AESUtil.decode(rv);
//        SecretMessage sm = JSON.parseObject(s,SecretMessage.class);
//        byte[] appSign = sm.getSign();//数字签名
//        Message msg = sm.getMessage();//明文消息
//        byte[] flag = msg.getFlag().getBytes();
//        //4.对数字签名：使用app公钥解密，拿到摘要
////        RSAUtil.verify(flag,userPublicKey,appSign);
//
//        //对明文消息：使用hash算法，拿到摘要
//
